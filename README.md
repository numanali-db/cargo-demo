...
handling rules from a vector-indexed knowledge base. Scenario: **Northwind Air Cargo**,
a fictional long-haul carrier (~200K tonnes / ~£250M revenue / 26K flights per year).

> **Data & scenario are synthetic.** All data is generated by `generate_cargo_data.py`
> (fixed seed) and calibrated to *public* air-cargo industry benchmarks. "Northwind Air
> Cargo" is a fictional carrier; no real customer data is used anywhere. See
> [`docs/execution-evidence.md`](docs/execution-evidence.md) for live run outputs, and the
> business [`docs/fe-bar-deck.md`](docs/fe-bar-deck.md).
...
- **A tool-using agent, not a single price model.** The 5-step pipeline
  (capacity → yield → competition → rules → LLM-drafted quote) exists so analysts can
  *see and override the reasoning*. A black-box price would be smaller but untrusted.
...
- **Lakebase for the RFQ inbox + quotes; lakehouse for analytics.** Operational
  ...
  Postgres; heavy aggregation (revenue, lanes, forwarders) stays on the SQL warehouse.
  Trade-off: two engines, each used for what it's best at. The app mints a short-lived
...
- **Genie for exploration, the agent for the transaction.** NL exploration of the book
  vs. the repeatable pricing decision — right tool per job.
- **Analytics windows anchored to `MAX(flight_date)`, not `CURRENT_DATE()`.** The dataset
  is static; anchoring keeps the KPI tiles populated whenever the demo is shown.

See also [`docs/ai-mindset.md`](docs/ai-mindset.md) for how AI was used to build this.
